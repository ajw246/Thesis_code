...
ask villages with [size != 1] [set size 1]
 ...
 ; commented out so model does not stop ; if total-polities = 1 [stop] ;stop if all villages are part of the same polity  ;;commented out because population can still grow, so more might still happen
  ...
  ;to get a number for how surrounded each village is by other villages:
  ; (minus 1 for count villages in-radius, because need to exclude the village itself)
  ...
  ask villages [
    ifelse (count patches with [pcolor = 52] in-radius moving-distance - 1) = 0
    ...
    [;if there some surrounding patches with more resources...
      ...
      ifelse (count patches with [pcolor = 52] in-radius moving-distance - 1) <= (((count patches in-radius moving-distance - 1) / 4) * 1)
      ...
      [;if there are more fertile patches...
       ...
       ifelse (count patches with [pcolor = 52] in-radius moving-distance - 1) <= (((count patches in-radius moving-distance - 1) / 2))
        ...
        [;if there are even more fertile patches about...
          ...
          ifelse (count patches with [pcolor = 52] in-radius moving-distance - 1) <= (((count patches in-radius moving-distance - 1) / 4) * 3)
  ...
  let total-green-resources fertile-land * ((count villages with [environmentally-circumscribed = 0] * 4) +
    (count villages with [environmentally-circumscribed = 1] * 3) +
    (count villages with [environmentally-circumscribed = 2] * 2) +
    (count villages with [environmentally-circumscribed = 3] ))

  let total-black-resources barren-land * ((count villages with [environmentally-circumscribed = 4] * 4) +
    (count villages with [environmentally-circumscribed = 3] * 3) +
    (count villages with [environmentally-circumscribed = 2] * 2) +
    (count villages with [environmentally-circumscribed = 1]) )

  let total-surrounding-resources total-green-resources + total-black-resources

  let highest-possible-resources (count villages * 4) * fertile-land

  set environmental-circumscription 1 - (total-surrounding-resources / highest-possible-resources)
